...
function loadForm() {
  var formNum = document.getElementById('formSel').value;
  if (formNum == 1) {//форма 1
    $.ajax("https://api.myjson.com/bins/1bos7h", { type: 'GET', dataType: 'json', success: builtForm, error: errorHandler });
  } else {//форма 2
    $.ajax("https://api.myjson.com/bins/gnvvh", { type: 'GET', dataType: 'json', success: builtForm, error: errorHandler });
  }
}
...
function builtForm(formDescr) {
  ...
  /* удаление уже загруженной формы*/
  var myNewForm = document.getElementById("myNewForm");
  if (myNewForm) {myNewForm.remove();}
  /*------*/
